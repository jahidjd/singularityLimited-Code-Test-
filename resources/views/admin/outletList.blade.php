    ...
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <h1>Outlet List</h1>
                    <a href="{{route('outlet')}}" class="btn" style="color: white">Add Outlet</a>
                    <table class="table table-bordered display " id="datatableDefault">
                        ...
                        <tbody>
                            @foreach ($outlet as $k=>$u)
                                <tr>
                                    <td>{{++$k}}</td>
                                    <td>{{$u->name}}</td>
                                    <td>{{$u->address}}</td>
                                    <td>{{$u->open_time}} AM</td>
                                    <td>{{$u->close_time}} PM</td>
                                    <td>{{$u->visit_date}}</td>
                                    
                                    <td>
                                        <a href="{{route('editOutlet',$u->id)}}" style="color: yellowgreen" class="btn"><i class="fas fa-edit"></i></a>
                                       <form action="{{route('deleteOutlet',$u->id)}}" method="post">
                                        @csrf
                                        @method('delete')
                                       <button type="submit" style="color: red" onclick="return confirm('Are you sure!')" class="btn"><i class="fas fa-trash"></i> </button>
                                      </form>
                                        <a href="{{route('report',$u->id)}}" class="btn btn-primary">Report</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
